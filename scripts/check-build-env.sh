...
if [[ -n "$SDK_DIR" ]]; then
    ...
    [[ -d "$SDK_DIR/platforms/android-36" ]] && check_pass "Platform: android-36" || check_fail "Platform: android-36 missing"
    [[ -d "$SDK_DIR/build-tools/35.0.0" ]] && check_pass "Build Tools: 35.0.0" || check_fail "Build Tools: 35.0.0 missing"
    [[ -d "$SDK_DIR/ndk/27.0.12077973" ]] && check_pass "NDK: 27.0.12077973" || check_fail "NDK: 27.0.12077973 missing"
...
if [[ "$FAIL" -eq 0 && "$WARN" -eq 0 ]]; then
    ...
    echo -e "  ${BLUE}  yes | sdkmanager --install \"platforms;android-36\" \"build-tools;35.0.0\" \"ndk;27.0.12077973\" \"cmake;3.31.6\" \"platform-tools\"${NC}"
